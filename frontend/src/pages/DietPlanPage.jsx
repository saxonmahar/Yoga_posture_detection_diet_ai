...
function DietPlanPage({ onNavigate, user }) {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          <h2 className="text-2xl font-bold mb-4">Please login to view your diet plan</h2>
          <button 
  ...
  const calculateDietPlan = () => {
    ...
    if (user?.stats?.goal === 'weight-loss') {
      calories = 1800
      carbs = 180
      protein = 140 // Higher protein for weight loss
    } else if (user?.stats?.goal === 'muscle-gain') {
      calories = 2200
      protein = 160
      carbs = 220
    }
    ...
    if (user?.stats?.activityLevel === 'active') {
      calories += 200
      protein += 10
    } else if (user?.stats?.activityLevel === 'sedentary') {
      calories -= 200
    }
  ...
  const waterIntake = user?.stats?.weight ? Math.round(user.stats.weight * 0.033) : 2
            ...
            <div className="bg-gradient-to-r from-blue-500/10 to-cyan-500/10 backdrop-blur-sm rounded-2xl p-6 border border-blue-500/20 card-hover">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-text-muted text-sm">Based on your weight: {user?.stats?.weight || 70}kg</p>
                </div>
            ...
            <div className="bg-card/50 backdrop-blur-sm rounded-2xl p-6 border border-white/10 card-hover">
              ...
              <div className="space-y-4">
                <div className="p-4 bg-accent/10 rounded-lg">
                  ...
                  <p className="text-sm text-text-muted">"Increase protein intake by 20g to support muscle recovery after your plank sessions."</p>
                </div>
                
                <div className="p-4 bg-green-500/10 rounded-lg">
                  <h4 className="font-medium mb-2 text-sm">Hydration Tip:</h4>
                  <p className="text-sm text-text-muted">"Drink 500ml water 30 minutes before your next workout for better performance."</p>
                </div>
